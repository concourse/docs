...
![](assets/2019-03-08-an-overview-of-authorization-in-concourse-3-4-and-5-01.jpg)
/// caption
Photo
credit [NASA HQ Photo](https://www.flickr.com/photos/nasahqphoto/6939110234/in/photolist-bzbKHC-ddzPE8-8L4EUM-9TVuB6-dtejJ5-9TYjmy-9TYjdG-bzFMVJ-73Q4go-8L4ECx-8L4ERi-rkkaR4-8L4EGP-bsVusD-9TYjAE-9TVug2-fAqNUc-9TVuuv-8Kg7xL-8Kg7qd-8L4Ed2-5Br7fF-dbAKWk-ri4hvf-dqDdxZ-9TVuLX-4gVHaW-23GVbV3-9hPW9E-9TVuDH-7rwXMD-2dCKGB3-JT9GBw-QAa9qu-GLvcYA-ddA1K7-STJ6JG-68xPrR-6F1k4t-nE7LmR-87aADq-8Unb63-9xQsSu-GEj6z-9hPML5-7zkhqs-5GZHzn-8knwUP-cCyc89-peP6GS/)
///
...
The revised [Concourse Auth & Teams docs][auth] is a great place to start when diving into
...
![](assets/2019-03-08-an-overview-of-authorization-in-concourse-3-4-and-5-02.png)

Every Concourse instance starts with a `main` team that must be configured against an Authentication Provider on
start-up. The `main` team is an admin team, meaning it can create teams, update other teams and view system-scoped
details on workers, containers, etc.

One of the tasks that only a `main` user can do is to create new teams via `set-team`. When creating the team, the
...
- Since Authentication Provider details are provided per-team, operators can set unique provders for each. A common
  use-case is to provision `Team 1` to authenticate against the USA-East-1 OAuth server and `Team2` to authenticate
  against the EMEA OAuth server.
- You can _stack_ Authentication Providers by supplying multiple parameters when applying `set-team`; e.g. a team can
  have both GitHub _and_ Basic Auth configured to authenticate users.
...
- Users logging into Concourse are whitelisted into all teams that match their provider membership. More on this later
...
3. The Operator will start Concourse and begin creating teams using the `fly set-team` command. Keeping in mind the auth
   ...
   See [Configuring Team Auth][managing] for more details.
...
6. When a login success response is received, Concourse will examine all of the teams/orgs the User belongs to under
...
under [Configuring Auth][config_auth].
...
the [Local Auth][local_auth] docs.
...
as using the parameters described in the `fly set-team` docs for [Configuring Team Auth][managing].

As with most `fly` commands, you can actually attach multiple users/teams across providers to a team. For example: if
you have GitHub and OAuth providers set up, a team owner could attach two teams (one from GitHub, one from OAuth) to the
team.

### Examples

![](assets/2019-03-08-an-overview-of-authorization-in-concourse-3-4-and-5-03.png)
/// caption
///
...
member of the `Pivotal` GitHub org she sees `Team All`, which is configured to allow all users under the `pivotal` org
on GitHub. She also sees Team 1 because it allows all users who are also memebers of `pivotal:cloud` on GitHub.

#### Operator Logs In

The Operator logs in using GitHub auth and...can see everything! Because the Operator is part of the `main` team, they
can see all teams. However, that does _not_ mean the Operator can see all the team pipelines. In this scenario, the
Operator can only see the `Main` and `Team All` team pipelines.
...
If you are an operator, and you need to figure out what the exact auth settings are, you can use the new `fly teams -d`
command. This will list the teams with details, including the users and groups whitelisted into that team
...
4. Was there a typo? Use `fly set-team -d` to look for the team in question and triple-check the spelling of usernames
   and teams
...
connect to two different identity providers of the same type.
...
A Concourse Admin is the same as today’s [admin user][admin]. Members of `main` team
...
- [Concourse Auth & Teams docs][auth]
...
[config_auth]: ../../../../docs/auth-and-teams/configuring/index.md

[local_auth]: ../../../../docs/auth-and-teams/configuring/local-user.md