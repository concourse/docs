...
If your Vault instance is signed with TLS by a local Certificate Authority, you can use the following environment
variable:

```properties
CONCOURSE_VAULT_CA_CERT=/etc/ca.crt
```

You'll also need to configure how the `web` node authenticates with Vault -
see [Authenticating with Vault](#configuring-authentication) for
