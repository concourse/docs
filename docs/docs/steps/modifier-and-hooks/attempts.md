...
??? info "`attempts`: [`number`](../../config-basics.md#number-schema)"
    ...
    ??? example "Retrying with a timeout"
        ...
        This semi-arbitrary decision was made because often things either succeed in a reasonable amount of time or fail
        due to hanging/flakiness. In this case it seems more useful to allow each attempt the allotted timeout rather 
